...
s3 = boto3.resource(service_name = "s3", endpoint_url = th.S3_ENDPOINT, aws_access_key_id = th.S3_ACCESS_KEY, aws_secret_access_key = th.S3_SECRET_KEY)
DT = date.today().strftime("%Y%m%d")

#DT вместо ds_nodash = None -
"""LOAD"""
...
with DAG(
    dag_id="transfermarkt_data_loading",
    start_date=datetime(2024, 1, 22),  # должна быть до реальной даты старта
    schedule="@daily",
    ...
    def export_to_gp():
        ...
        #     postgres_conn_id="GP",
        #     #sql="select stg.load_clubs('{{ds_nodash}}');",
        #     sql='sql/load_clubs_to_gp.sql',
        # )
        ...
        #     postgres_conn_id="GP",
        #     #sql="select stg.load_clubs('{{ds_nodash}}');",
        #     sql='sql/load_club_profiles_to_gp.sql',
        # )

        # load_club_players_to_gp = PostgresOperator(
        #     task_id="load_clubs_players_to_gp",
        #     postgres_conn_id="GP",
        #     #sql="select stg.load_clubs('{{ds_nodash}}');",
        #     sql='sql/load_club_players_to_gp.sql',
        # )
        ...
        load_player_other_positions_to_gp = PostgresOperator(
            ...
            postgres_conn_id="GP",
            sql="select stg.load_player_other_positios('{{ds}}');",
        ...
        load_player_citizenship_to_gp = PostgresOperator(
            ...
            postgres_conn_id="GP",
            sql="select stg.load_player_citizenship('{{ds}}');",
        ...
        load_clubs_to_gp
        load_club_profiles_to_gp
        load_club_players_to_gp
        load_player_profiles_to_gp
        load_player_other_positions_to_gp
        load_player_citizenship_to_gp
        load_player_market_values_to_gp
